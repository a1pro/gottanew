...
=> $this->serializeRequest($sessionRequest))
  ...
  public function assignableCoaches()
  {
      $coaches = Coach::query()
          ...
          ->with('user:id,email')
          ->orderBy('name')
          ->get(['id', 'user_id', 'name', 'title', 'timezone',
'notification_email', 'available_now']);
  ...
  {
      $admin = $request->user();
      $sessionRequest = SessionRequest::query()->with(['client',
'preferredCoach'])->findOrFail($id);
      ...
      $validated = $request->validate([
          ...
          'viewer_timezone' => ['required', 'string', 'max:100', new
TimezoneIdentifier()],
...
'UTC');
      ...
      $session = DB::transaction(function () use ($sessionRequest, $coach,
$admin, $validated, $scheduledStartUtc, $room, $viewerTimezone) {
          ...
          return $session->load(['coach.user', 'client', 'videoDetail',
'stateLogs']);
      });

      $sessionRequest->load(['client', 'assignedCoach.user',
'approvedSession', 'reviewer']);
      $this->notificationService->sessionRequestApproved($sessionRequest,
$session);
  ...
  public function reject(Request $request, int $id)
  {
      ...
      return $this->success($this->serializeRequest($sessionRequest->fresh(['client', 'preferredCoach', 'assignedCoach.user', 'approvedSession',
