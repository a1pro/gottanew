...
empty($user->email_verified_at)),
  ...
  public function update(Request $request)
  {
      ...
      $validated = $request->validate([
          ...
          'timezone' => ['nullable', 'string', 'max:100', new
TimezoneIdentifier()],
...
Timezone::normalize($validated['timezone'], $coach->timezone ?: 'UTC') : $coach->timezone,
          ], fn ($value) => $value !== null));
      }
  ...
  public function completeOnboarding(Request $request)
  {
      $validated = $request->validate([
          ...
          'timezone' => ['required', 'string', 'max:100', new
TimezoneIdentifier()],
        ...
        try {
            ...
            $coach = Coach::updateOrCreate(
                ...
                [
                    ...
                    'similar_experiences' => [],
                    'timezone' => Timezone::normalize($validated['timezone'],
'UTC'),
                     'notification_email' => $user->email,
  ...
  private function formatLegalPayload(Profile $profile): array
  {
    ...
    private function resolveLegalUpdates(Profile $profile, array $payload):
array
    {
        $updates = [];
         ...
         if (!empty($payload['accept_privacy_policy'])) {
             $updates['legal_version'] = self::LEGAL_VERSION;
             $updates['privacy_policy_accepted_at'] = now();
         }

         if (!empty($payload['accept_coaching_disclaimer'])) {
             $updates['legal_version'] = self::LEGAL_VERSION;
             $updates['coaching_disclaimer_accepted_at'] = now();
         }

         if (!empty($payload['acknowledge_coach_independence'])) {
             $updates['legal_version'] = self::LEGAL_VERSION;
